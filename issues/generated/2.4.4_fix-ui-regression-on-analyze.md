...
- [x] After clicking Analyze, the app does not switch to the legacy design (no “flash”/re-render into old layout/styling).
- [x] Analyze-triggered reruns preserve UI configuration (layout, theme/styling injection, component hierarchy) and session state.
- [x] Root cause identified and documented (e.g., conditional render paths, duplicated layout code, `st.rerun()`/session state interactions).
