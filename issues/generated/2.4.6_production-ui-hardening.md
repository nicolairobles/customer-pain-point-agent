...
## Acceptance Criteria
- [x] Convert the query input + Analyze flow to a `st.form` submission (no accidental reruns while typing).
- [x] Introduce a single “production mode” setting and use it to gate debug-only UI (default: production mode on / debug UI off).
- [x] Update debug panel gating to use the centralized setting (not ad-hoc env parsing in multiple places).
- [x] Add a release UI QA checklist under `docs/ui/` (cross-browser, responsive, rerun stability, error/empty states).
